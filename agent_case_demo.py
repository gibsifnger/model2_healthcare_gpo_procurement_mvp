...
def extract_facts(text: str) -> Facts:
    t = norm(text)
    request_no = first_match(t, [r"요청번호\s*[^0-9]{0,20}([0-9]{7,10})", r"/\s*([0-9]{7,10})\s*기\s*안"])
    item_type = first_match(t, [r"품목유형\s*([A-Za-z가-힣]+)"])
    supplier = first_match(t, [r"대상업체\s*[:：]?\s*((?:\(주\))?[^0-9]{2,30}?)(?=\s*[0-9]\.|\s*용도)", r"제조사명.*?((?:\(주\))?아이비리더스)"])
    item_name = first_match(t, [r"(GRID\(IBSHEET\)\s*유지보수)", r"제품명\s*(IB\s*Sheet\s*8)"])
    amount = first_match(t, [r"금액\(VAT 포함\)\s*[:：]?\s*연간\s*([0-9,]+원)", r"견\s*적\s*금\s*액\s*[:：]?\s*([0-9,]+\s*원)", r"예상구입금액 합계.*?([0-9,]+)"])
    start = first_match(t, [r"계약기간 시작일\s*([0-9]{4}-[0-9]{2}-[0-9]{2})", r"유지보수 기간\s*[:：]?\s*([0-9]{4}년\s*[0-9]{1,2}월\s*[0-9]{1,2}일)"])
    end = first_match(t, [r"계약기간 종료일\s*([0-9]{4}-[0-9]{2}-[0-9]{2})", r"~\s*([0-9]{4}년\s*[0-9]{1,2}월\s*[0-9]{1,2}일)\s*\(12개월\)"])
...
def decide(s: Signals, purchase_type: str) -> Decision:
    ...
    if not (s.mdvan_ready and s.fee_ready and s.quotation_ready):
        missing = []
        if not s.mdvan_ready: missing.append("MDVAN 가입/등록 가능 여부")
        if not s.fee_ready: missing.append("수수료 적용 수용 여부")
        if not s.quotation_ready: missing.append("견적 제출 가능 여부/시점")
...
def mdvan_guide(action: str, supplier: str, prior_same: bool) -> List[Tuple[str, str]]:
    ...
    if action == "ASK_SUPPLIER":
        return [("현재 MDVAN", "아직 수의시담 발송하지 않음"), ("업체 확인", f"{supplier}만 수행 가능한 이유 및 서면 근거 요청"), ("동시 확인", "MDVAN 가입/등록, 수수료 수용, 견적 제출 가능 여부"), ("가격검토 방향", "기안에 '작년 금액과 동일'이 있어 종전가격 비교 자료로 활용 가능" if prior_same else "종전가격 등 가격검증 근거 확인")]
    if action == "CHECK_SUPPLIER_READY":
        return [("현재 MDVAN", "수의시담 발송 전 사전 확인"), ("공급사", supplier), ("확인 1", "MDVAN 가입/등록 가능 여부"), ("확인 2", "IT 수수료 적용 수용 여부"), ("확인 3", "견적 제출 가능 시점")]
    if action == "SEND_NEGOTIATION":
        return [("메뉴", "MDVAN > 수의시담/가격조사 발송 화면"), ("공급사", supplier), ("첨부", "사양/요청자료 및 견적에 필요한 문서"), ("이지메모", "해당 구매 건 관련 수의시담 요청드립니다. 첨부자료 확인 후 견적 제출 부탁드립니다."), ("실행", "공급사·첨부·수수료 조건 확인 후 담당자가 발송")]
    if action == "PREPARE_APPROVAL":
        return [("다음 화면", "구매결재 작성"), ("가격비교", "종전가격과 최종견적 비교" if prior_same else "가용한 가격근거와 최종견적 비교"), ("작성", "9. 가격조사기준 / 10. 특이사항 정리"), ("실행", "금액·수수료·계약방법 확인 후 담당자가 상신")]
...
if demo_mode:
    ...
    # 실제 파일이 올라오면 데모에서도 Case 기본정보는 실제 문서에서 사용한다.
    if text:
...
# 실제 KDWEZ 서류에서 자동 추출한 핵심 정보
st.markdown("### Case 기본정보")
fact_items = [
    ("Case 번호", case_id or "미입력"), ("요청번호", facts.request_no), ("품목유형", facts.item_type),
    ("공급사", facts.supplier), ("예상/견적금액", facts.amount), ("계약기간", facts.contract_period),
]
...
if facts.title != "미확인": st.caption(f"문서 제목: {facts.title}")
...
with right:
    st.markdown('<div class="card"><div class="card-title">EVIDENCE CHECK</div>', unsafe_allow_html=True)
    evidence = [("구매기안/요청자료",signals.purchase_doc),("기존 견적",signals.quote),("단일 공급사 근거",signals.exclusive_reason),("MDVAN 가입/등록",signals.mdvan_ready),("수수료 수용",signals.fee_ready),("견적 제출 가능",signals.quotation_ready),("최종 견적",signals.final_quote),("종전가격 동일 근거",signals.prior_price_same)]
    for n,ok in evidence:
        st.markdown(f'<div class="ev"><div class="ev-name">{n}</div><span class="pill {"yes" if ok else "no"}">{"확인" if ok else "미확인"}</span></div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    if missing:
        st.markdown("#### 추가 확인 필요")
        for m in missing: st.markdown(f"- **{m}**")
    st.markdown("#### Guardrail")
    for r in base.rules: st.markdown(f'<div class="rule">{r}</div>', unsafe_allow_html=True)
    st.markdown('<span class="pill warn">Human approval required</span>', unsafe_allow_html=True)

if names:
    st.markdown("### 읽은 실제 파일")
    for n in names: st.markdown(f'<div class="source">{n}</div>', unsafe_allow_html=True)
...
with st.expander("기술 로그 / 추출 텍스트", False):
    ...
    if text: st.text_area("문서 추출 텍스트", text[:15000], height=260)
